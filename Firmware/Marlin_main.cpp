...
const char errormagic[]
PROGMEM = "Error:";
const char echomagic[]
PROGMEM = "echo:";
...
void crashdet_detected(uint8_t mask) {
  ...
  if (mask & X_AXIS_MASK) {
    ...
    eeprom_update_word((uint16_t *) EEPROM_CRASH_COUNT_X_TOT,
                       eeprom_read_word((uint16_t *) EEPROM_CRASH_COUNT_X_TOT) + 1);
  }
  if (mask & Y_AXIS_MASK) {
    eeprom_update_byte((uint8_t *) EEPROM_CRASH_COUNT_Y, eeprom_read_byte((uint8_t *) EEPROM_CRASH_COUNT_Y) + 1);
    eeprom_update_word((uint16_t *) EEPROM_CRASH_COUNT_Y_TOT,
                       eeprom_read_word((uint16_t *) EEPROM_CRASH_COUNT_Y_TOT) + 1);
  }
...
enum MeshLevelingState {
    MeshReport, MeshStart, MeshNext, MeshSet
};
#endif
  ...
  switch (FW_DEV_VERSION) {
    case (FW_VERSION_ALPHA):
      lcd_show_fullscreen_message_and_wait_P(
              _i("You are using firmware alpha version. This is development version. Using this version is not recommended and may cause printer damage."));
      break;////MSG_FW_VERSION_ALPHA c=20 r=8
    case (FW_VERSION_BETA):
      lcd_show_fullscreen_message_and_wait_P(
              _i("You are using firmware beta version. This is development version. Using this version is not recommended and may cause printer damage."));
...
/*	if (mcu & 1) SERIAL_ECHOLNRPGM(MSG_POWERUP);
  ...
  if (!(hw_changed &
        0b10)) { //if printer version wasn't changed, check for eeprom version and retrieve settings from eeprom in case that version wasn't changed
...
//	lang_reset();
  ...
  switch (hw_changed) {
    ...
    case (0b01):
      lcd_show_fullscreen_message_and_wait_P(
              _i("Warning: motherboard type changed.")); ////MSG_CHANGED_MOTHERBOARD c=20 r=4
    ...
    case (0b11):
      lcd_show_fullscreen_message_and_wait_P(
              _i("Warning: both printer type and motherboard type changed.")); ////MSG_CHANGED_BOTH c=20 r=4
      eeprom_write_word((uint16_t *) EEPROM_PRINTER_TYPE, PRINTER_TYPE);
      eeprom_write_word((uint16_t *) EEPROM_BOARD_TYPE, MOTHERBOARD);
      break;
    default:
  ...
  if (!previous_settings_retrieved) {
    lcd_show_fullscreen_message_and_wait_P(
            _i("Old settings found. Default PID, Esteps etc. will be set.")); //if EEPROM version or printer type was changed, inform user that default setting were loaded////MSG_DEFAULT_SETTINGS_LOADED c=20 r=5
  ...
  if (eeprom_read_byte((uint8_t *) EEPROM_WIZARD_ACTIVE) ==
      0) { //dont show calibration status messages if wizard is currently active
    if (calibration_status() == CALIBRATION_STATUS_ASSEMBLED ||
        calibration_status() == CALIBRATION_STATUS_UNKNOWN ||
        calibration_status() == CALIBRATION_STATUS_XYZ_CALIBRATION) {
      // Reset the babystepping values, so the printer will not move the Z axis up when the babystepping is enabled.
      eeprom_update_word(reinterpret_cast<uint16_t *>(&(EEPROM_Sheets_base->s[(eeprom_read_byte(
              &(EEPROM_Sheets_base->active_sheet)))].z_offset)), 0);
      // Show the message.
      ...
      lcd_update_enable(true);
    } else if (calibration_status() == CALIBRATION_STATUS_CALIBRATED &&
               eeprom_read_byte((unsigned char *) EEPROM_TEMP_CAL_ACTIVE) && calibration_status_pinda() == false) {
...
*/
  ...
  wdt_enable(WDTO_4S);
#endif //WATCHDOG
  Sound_MakeCustom(400,200,false);
  Sound_MakeCustom(400,300,false);
  Sound_MakeCustom(500,400,false);
}
...
void loop() {
  ...
  if (isPrintPaused && saved_printing_type ==
                       PRINTING_TYPE_USB) //keep believing that usb is being printed. Prevents accessing dangerous menus while pausing.
  {
    is_usb_printing = true;
  }
...
void retract(bool retracting, bool swapretract = false) {
  if (retracting && !retracted[active_extruder]) {
    ...
    current_position[E_AXIS] +=
            (swapretract ? retract_length_swap : cs.retract_length) * float(extrudemultiply) * 0.01f;
    ...
    plan_set_position_curposXYZE();
    current_position[E_AXIS] -=
            (swapretract ? (retract_length_swap + retract_recover_length_swap) : (cs.retract_length +
                                                                                  cs.retract_recover_length)) *
            float(extrudemultiply) * 0.01f;
    plan_set_e_position(current_position[E_AXIS]);
    float oldFeedrate = feedrate;
...
static void
gcode_G28(bool home_x_axis, long home_x_value, bool home_y_axis, long home_y_value, bool home_z_axis, long home_z_value,
          bool calib, bool without_mbl)
#else
...
#endif //TMC2130
{
  ...
  if (home_z) {
    ...
    world2machine(pgm_read_float(bed_ref_points_4), pgm_read_float(bed_ref_points_4 + 1), destination[X_AXIS],
                  destination[Y_AXIS]);
    world2machine_reset();
...
static T gcode_M600_filament_change_z_shift() {
  ...
  static_assert(Z_MAX_POS < (255 - FILAMENTCHANGE_ZADD),
                "Z-range too high, change the T type from uint8_t to uint16_t");
...
static void
gcode_M600(bool automatic, float x_position, float y_position, float z_shift, float e_shift, float /*e_shift_late*/) {
  st_synchronize();
  float lastpos[4];
  ...
  if (!mmu_enabled) {
    ...
    lcd_change_fil_state = lcd_show_fullscreen_message_yes_no_and_wait_P(_i("Was filament unload successful?"),
                                                                         false,
                                                                         true); ////MSG_UNLOAD_SUCCESSFUL c=20 r=2
    if (lcd_change_fil_state == 0) {
      ...
      st_synchronize();
      lcd_show_fullscreen_message_and_wait_P(
              _i("Please open idler and remove filament manually."));////MSG_CHECK_IDLER c=20 r=4
    }
  }
  ...
  if (mmu_enabled) {
    if (!automatic) {
      if (saved_printing)
        mmu_eject_filament(mmu_extruder,
 ...
 */
  if (code_seen(
          "M117")) { //moved to highest priority place to be able to to print strings which includes "G", "PRUSA" and "^"
...
*/
      ...
      if (code_seen('D')) {
        ...
        nDiameter = (uint16_t) (code_value() * 1000.0 + 0.5); // [,um]
        eeprom_update_byte((uint8_t *) EEPROM_NOZZLE_DIAMETER,
                           (uint8_t) ClNozzleDiameter::_Diameter_Undef); // for correct synchronization after farm-mode exiting
...
if(eSoundMode!=e_SOUND_MODE_SILENT)
          ...
          if (total_filament_used > ((current_position[E_AXIS] - destination[E_AXIS]) *
                                     100)) { //protection against total_filament_used overflow
          ...
          if (cs.autoretract_enabled)
            if (!(code_seen('X') || code_seen('Y') || code_seen('Z')) && code_seen('E')) {
              ...
              if ((echange < -MIN_RETRACT && !retracted[active_extruder]) || (echange > MIN_RETRACT &&
                                                                              retracted[active_extruder])) { //move appears to be an attempt to retract or recover
  ...
  */
      case 76: {
        ...
        lcd_show_fullscreen_message_and_wait_P(
                _i("Stable ambient temperature 21-26C is needed a rigid stand is required."));////MSG_TEMP_CAL_WARNING c=20 r=4
        bool result = lcd_show_fullscreen_message_yes_no_and_wait_P(_T(MSG_STEEL_SHEET_CHECK), false, false);
        ...
        if ((current_temperature_pinda > 35) && (farm_mode == false)) {
          ...
          plan_buffer_line_curposXYZE(3000 / 60);
          if (lcd_wait_for_pinda(35) ==
              false) { //waiting for PINDA probe to cool, if this takes more then time expected, temp. cal. fails
...
//			setTargetHotend(200, 0);
        ...
        eeprom_update_byte((uint8_t *) EEPROM_CALIBRATION_STATUS_PINDA,
                           0); //invalidate temp. calibration in case that in will be aborted during the calibration process
...
//				setTargetHotend(255, 0);
          ...
          printf_P(_N("\nPINDA temperature: %.1f Z shift (mm): %.3f"), current_temperature_pinda,
                   current_position[Z_AXIS] - zero_z);
      ...
      case_G80:
      {
        ...
        while (mesh_point != nMeasPoints * nMeasPoints) {
          ...
          if ((ix == 0) && (iy == 0)) current_position[Z_AXIS] = MESH_HOME_Z_SEARCH;
            ...
            current_position[Z_AXIS] += 2.f /
                                        nMeasPoints; //use relative movement from Z coordinate where PINDa triggered on previous point. This makes calibration faster.
          ...
          if (!find_bed_induction_sensor_point_z((has_z && mesh_point > 0) ? z0 - Z_CALIBRATION_THRESHOLD : -10.f,
                                                 nProbeRetry)) { //if we have data from z calibration max allowed difference is 1mm for each point, if we dont have data max difference is 10mm from initial point
            printf_P(_T(MSG_BED_LEVELING_FAILED_POINT_LOW));
            break;
          }
          if (init_z_bckp - current_position[Z_AXIS] <
              0.1f) { //broken cable or initial Z coordinate too low. Go to MESH_HOME_Z_SEARCH and repeat last step (z-probe) again to distinguish between these two cases.
          ...
          if (has_z && fabs(z0 - current_position[Z_AXIS]) >
                       Z_CALIBRATION_THRESHOLD) { //if we have data from z calibration, max. allowed difference is 1mm for each point
...
//			#endif // SUPPORT_VERBOSITY
          mbl.set_z(ix, iy,
                    current_position[Z_AXIS] - offset_z); //store measured z values z_values[iy][ix] = z - offset_z;
...
//               plan_set_z_position(MESH_HOME_Z_SEARCH); // is not necessary ('do-while' loop always ends at the expected Z-position)
          ...
          gcode_G28(true, true,
                    true);       // X & Y & Z-homing (must be after individual Z-homing (problem with spool-holder)!)
        ...
        for (uint8_t i = 0; i < 4; ++i) {
          ...
          if (code_seen(codes[i]))
            ...
            unsigned char *addr = (i < 2) ?
                                  ((i == 0) ? (unsigned char *) EEPROM_BED_CORRECTION_LEFT
                                            : (unsigned char *) EEPROM_BED_CORRECTION_RIGHT) :
                                  ((i == 2) ? (unsigned char *) EEPROM_BED_CORRECTION_FRONT
...
//		SERIAL_ECHOLNPGM("Go home finished");
        ...
        if ((degHotend(active_extruder) > EXTRUDE_MINTEMP) &&
            eeprom_read_byte((unsigned char *) EEPROM_TEMP_CAL_ACTIVE) && calibration_status_pinda() &&
   ...
   */
	...
	*/
...
#if defined(TEMP_BED_PIN) && TEMP_BED_PIN > -1
        {
          ...
          KEEPALIVE_STATE(NOT_BUSY);
          while ((target_direction) && (!cancel_heatup) ? (isHeatingBed()) : (isCoolingBed() &&
          ...
          for (int8_t i = 0; i < NUM_AXIS; i++) {
            if (code_seen(axis_codes[i])) {
              if (i == E_AXIS) { // E
                ...
                if (value < 20.0) {
                  float factor =
                          cs.axis_steps_per_unit[i] / value; // increase e constants if M92 E14 is given for netfab.
...
//			NOMORE(host_keepalive_interval, 60);
    ...
    */
        case 860: {
          ...
          while (((!is_pinda_cooling) && (!cancel_heatup) && (current_temperature_pinda < set_target_pinda)) ||
                 (is_pinda_cooling && (current_temperature_pinda > set_target_pinda))) {
        ...
        case 350: {
          ...
          for (int i = 0; i < NUM_AXIS; i++) {
            if (code_seen(axis_codes[i])) {
              ...
              res_valid |= (i != E_AXIS) &&
                           ((res_new == 1) || (res_new == 2) || (res_new == 4)); // resolutions valid for X Y Z only
  ...
  */
    ...
    *(strchr_pointer + index) = tolower(*(strchr_pointer + index));
    ...
    if ((*(strchr_pointer + index) < '0' || *(strchr_pointer + index) > '4') && *(strchr_pointer + index) != '?' &&
        *(strchr_pointer + index) != 'x' && *(strchr_pointer + index) != 'c') {
...
void mesh_plan_buffer_line(const float &x, const float &y, const float &z, const float &e, const float &feed_rate,
...
#ifdef MESH_BED_LEVELING
    mesh_plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS],
                          feedrate * feedmultiply * (1. / (60.f * 100.f)), active_extruder);
#else
...
void manage_inactivity_IR_ANALOG_Check(uint16_t &nFSCheckCount, ClFsensorPCB isVersion, ClFsensorPCB switchTo,
        ...
        (!bInhibitFlag)) //M600 not in progress, preHeat @ autoLoad menu not active, Support::ExtruderInfo/SensorInfo menu not active
    {
      if (!moves_planned() && !IS_SD_PRINTING && !is_usb_printing && (lcd_commands_type != LcdCommands::Layer1Cal) &&
        ...
        if (minVolt >= IRsensor_Ldiode_TRESHOLD && minVolt <= IRsensor_Lmax_TRESHOLD
          ...
          manage_inactivity_IR_ANALOG_Check(nFSCheckCount, ClFsensorPCB::_Old, ClFsensorPCB::_Rev04,
                                            _i("FS v0.4 or newer")); ////c=18
        }
          ...
          manage_inactivity_IR_ANALOG_Check(nFSCheckCount, ClFsensorPCB::_Rev04, oFsensorPCB = ClFsensorPCB::_Old,
                                            _i("FS v0.3 or older")); ////c=18
        }
#endif // IR_SENSOR_ANALOG
...
void save_statistics(unsigned long _total_filament_used,
                     unsigned long _total_print_time) //_total_filament_used unit: mm/100; print time in s
{
  if (eeprom_read_byte((uint8_t *) EEPROM_TOTALTIME) == 255 &&
      eeprom_read_byte((uint8_t *) EEPROM_TOTALTIME + 1) == 255 &&
      eeprom_read_byte((uint8_t *) EEPROM_TOTALTIME + 2) == 255 &&
      eeprom_read_byte((uint8_t *) EEPROM_TOTALTIME + 3) == 255) {
    eeprom_update_dword((uint32_t *) EEPROM_TOTALTIME, 0);
    eeprom_update_dword((uint32_t *) EEPROM_FILAMENTUSED, 0);
  }
  ...
  eeprom_update_dword((uint32_t *) EEPROM_TOTALTIME,
                      _previous_time + (_total_print_time / 60)); //EEPROM_TOTALTIME unit: min
...
static void wait_for_heater(long codenum, uint8_t extruder) {
  ...
  while ((!cancel_heatup) && ((residencyStart == -1) ||
                              (residencyStart >= 0 &&
                               (((unsigned int) (_millis() - residencyStart)) < (TEMP_RESIDENCY_TIME * 1000UL))))) {
#else
    while (target_direction ? (isHeatingHotend(tmp_extruder)) : (isCoolingHotend(tmp_extruder) && (CooldownNoWait == false))) {
#endif //TEMP_RESIDENCY_TIME
    if ((_millis() - codenum) >
        1000UL) { //Print Temp Reading and remaining time every 1 second while heating up/cooling down
      if (!farm_mode) {
        SERIAL_PROTOCOLPGM("T:");
        SERIAL_PROTOCOL_F(degHotend(extruder), 1);
        SERIAL_PROTOCOLPGM(" E:");
    ...
    if ((residencyStart == -1 && target_direction &&
         (degHotend(extruder) >= (degTargetHotend(extruder) - TEMP_WINDOW))) ||
...
float temp_comp_interpolation(float inp_temperature) {
  ...
  for (i = 0; i < n - 1; i++)
    if ((x[i] <= inp_temperature && inp_temperature <= x[i + 1]) || (i == n - 2 && inp_temperature > x[i + 1])) {
      ...
      d = f[i];
      sum = a * pow((inp_temperature - x[i]), 3) + b * pow((inp_temperature - x[i]), 2) + c * (inp_temperature - x[i]) +
            d;
    }
...
void uvlo_() {
  ...
  for (int8_t mesh_point = 0; mesh_point < MESH_NUM_X_POINTS * MESH_NUM_Y_POINTS; ++mesh_point) {
    ...
    eeprom_update_word((uint16_t *) (EEPROM_UVLO_MESH_BED_LEVELING_FULL + 2 * mesh_point),
...
void uvlo_tiny() {
  ...
  if (abs(current_position[Z_AXIS] - eeprom_read_float((float *) (EEPROM_UVLO_TINY_CURRENT_POSITION_Z))) >=
      1.f / cs.axis_steps_per_unit[Z_AXIS]) {
    // Clean the input command queue, inhibit serial processing using saved_printing
    cmdqueue_reset();
    card.sdprinting = false;
    saved_printing = true;
...
//	    current_position[axis] = st_get_position_mm(axis);
  ...
  plan_buffer_line(saved_pos[X_AXIS], saved_pos[Y_AXIS], current_position[Z_AXIS], saved_pos[E_AXIS] - e_move,
                   homing_feedrate[Z_AXIS] / 13, active_extruder);
  //then move Z
  plan_buffer_line(saved_pos[X_AXIS], saved_pos[Y_AXIS], saved_pos[Z_AXIS], saved_pos[E_AXIS] - e_move,
                   homing_feedrate[Z_AXIS] / 13, active_extruder);
...
void print_physical_coordinates() {
  printf_P(_N("physical coordinates: (%.3f, %.3f, %.3f)\n"), st_get_position_mm(X_AXIS), st_get_position_mm(Y_AXIS),
...
void M600_wait_for_user(float HotendTempBckp) {
  ...
  while (!(wait_for_user_state == 0 && lcd_clicked())) {
    ...
    SET_OUTPUT(BEEPER);
    if (counterBeep == 0) {
      if ((eSoundMode == e_SOUND_MODE_BLIND) || (eSoundMode == e_SOUND_MODE_LOUD) ||
      ...
      case 0: //nozzle is hot, waiting for user to press the knob to unload filament
        ...
        if (_millis() > waiting_start_time + (unsigned long) M600_TIMEOUT * 1000) {
          lcd_display_message_fullscreen_P(
                  _i("Press knob to preheat nozzle and continue."));////MSG_PRESS_TO_PREHEAT c=20 r=4
