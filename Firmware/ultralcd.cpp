    ...
    switch (heating_status) {
      ...
      default:
        break;
             ...
             (custom_message_type == CustomMsg::Status)) { // If printing from SD, show what we are printing
...
void
lcd_status_screen()                          // NOT static due to using inside "Marlin_main" module ("manage_inactivity()")
{
  if (firstrun == 1) {
    firstrun = 0;
...
void
lcd_menu_extruder_info()                     // NOT static due to using inside "Marlin_main" module ("manage_inactivity()")
{
...
#if defined(TMC2130) && defined(FILAMENT_SENSOR)
static const char failStatsFmt[]
PROGMEM = "%S\n" " %-16.16S%-3d\n" " %-16.16S%-3d\n" " %-7.7SX %-3d  Y %-3d";
...
static void lcd_menu_AutoLoadFilament() {
  ...
  lcd_display_message_fullscreen_nonBlocking_P(
          _i("Autoloading filament is active, just press the knob and insert filament..."),
          nlines);////MSG_AUTOLOADING_ENABLED c=20 r=4
  menu_back_if_clicked();
}
...
static void lcd_babystep_z() {
  ...
  if (lcd_draw_update) {
    ...
    lcd_print(buffer.c);
    lcd_set_cursor(0, 1);
    menu_draw_float13(_i("Adjusting Z:"),
                      _md->babystepMemMMZ); ////MSG_BABYSTEPPING_Z c=15 Beware: must include the ':' as its last character
  }
...
void lcd_adjust_bed(void) {
  ...
  MENU_BEGIN();
    ...
    MENU_ITEM_EDIT_int3_P(_i("Left side [um]"), &_md->left, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);////MSG_BED_CORRECTION_LEFT c=14 r=1
    MENU_ITEM_EDIT_int3_P(_i("Right side[um]"), &_md->right, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);////MSG_BED_CORRECTION_RIGHT c=14 r=1
    MENU_ITEM_EDIT_int3_P(_i("Front side[um]"), &_md->front, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);////MSG_BED_CORRECTION_FRONT c=14 r=1
    MENU_ITEM_EDIT_int3_P(_i("Rear side [um]"), &_md->rear, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);////MSG_BED_CORRECTION_REAR c=14 r=1
...
int8_t lcd_show_multiscreen_message_yes_no_and_wait_P(const char *msg, bool allow_timeouting,
                                                      bool default_yes) //currently just max. n*4 + 3 lines supported (set in language header files)
{
  return lcd_show_multiscreen_message_two_choices_and_wait_P(msg, allow_timeouting, default_yes, _T(MSG_YES),
                                                             _T(MSG_NO));
}
...
void lcd_bed_calibration_show_result(BedSkewOffsetDetectionResultType result, uint8_t point_too_far_mask) {
  ...
  if (result == BED_SKEW_OFFSET_DETECTION_POINT_NOT_FOUND) {
    ...
    if (point_too_far_mask == 0)
      ...
      msg = _i(
              "XYZ calibration failed. Front calibration points not reachable.");////MSG_BED_SKEW_OFFSET_DETECTION_FAILED_FRONT_BOTH_FAR c=20 r=8
    else if ((point_too_far_mask & 1) == 0)
      // The right and maybe the center point out of reach.
      msg = _i(
              "XYZ calibration failed. Right front calibration point not reachable.");////MSG_BED_SKEW_OFFSET_DETECTION_FAILED_FRONT_RIGHT_FAR c=20 r=8
    else
      // The left and maybe the center point out of reach.
      msg = _i(
              "XYZ calibration failed. Left front calibration point not reachable.");////MSG_BED_SKEW_OFFSET_DETECTION_FAILED_FRONT_LEFT_FAR c=20 r=8
    ...
    if (point_too_far_mask != 0) {
      if (point_too_far_mask == 2 || point_too_far_mask == 7)
        // Only the center point or all the three front points.
        msg = _i(
                "XYZ calibration compromised. Front calibration points not reachable.");////MSG_BED_SKEW_OFFSET_DETECTION_WARNING_FRONT_BOTH_FAR c=20 r=8
      else if ((point_too_far_mask & 1) == 0)
        // The right and maybe the center point out of reach.
        msg = _i(
                "XYZ calibration compromised. Right front calibration point not reachable.");////MSG_BED_SKEW_OFFSET_DETECTION_WARNING_FRONT_RIGHT_FAR c=20 r=8
      else
        // The left and maybe the center point out of reach.
        msg = _i(
                "XYZ calibration compromised. Left front calibration point not reachable.");////MSG_BED_SKEW_OFFSET_DETECTION_WARNING_FRONT_LEFT_FAR c=20 r=8
      lcd_show_fullscreen_message_and_wait_P(msg);
    }
    if (point_too_far_mask == 0 || result > 0) {
      switch (result) {
        ...
        case BED_SKEW_OFFSET_DETECTION_PERFECT:
          msg = _i(
                  "XYZ calibration ok. X/Y axes are perpendicular. Congratulations!");////MSG_BED_SKEW_OFFSET_DETECTION_PERFECT c=20 r=8
          break;
        case BED_SKEW_OFFSET_DETECTION_SKEW_MILD:
          msg = _i(
                  "XYZ calibration all right. X/Y axes are slightly skewed. Good job!");////MSG_BED_SKEW_OFFSET_DETECTION_SKEW_MILD c=20 r=8
          break;
        case BED_SKEW_OFFSET_DETECTION_SKEW_EXTREME:
          msg = _i(
                  "XYZ calibration all right. Skew will be corrected automatically.");////MSG_BED_SKEW_OFFSET_DETECTION_SKEW_EXTREME c=20 r=8
          break;
      }
      lcd_show_fullscreen_message_and_wait_P(msg);
    }
...
void
lcd_menu_show_sensors_state()                // NOT static due to using inside "Marlin_main" module ("manage_inactivity()")
{
  lcd_timeoutToStatus.stop();
...
static void lcd_crash_mode_info() {
  ...
  if ((tim + 1000) < _millis()) {
    ...
    fputs_P(_i("Crash detection can\nbe turned on only in\nNormal mode"),
            lcdout);////MSG_CRASH_DET_ONLY_IN_NORMAL c=20 r=4
    tim = _millis();
  }
  menu_back_if_clicked();
}
...
static void lcd_crash_mode_info2() {
  ...
  if ((tim + 1000) < _millis()) {
    lcd_clear();
    fputs_P(_i("WARNING:\nCrash detection\ndisabled in\nStealth mode"),
            lcdout);////MSG_CRASH_DET_STEALTH_FORCE_OFF c=20 r=4
    tim = _millis();
  }
  menu_back_if_clicked();
}
...
static void lcd_filament_autoload_info() {
  ...
  if ((tim + 1000) < _millis()) {
    lcd_display_message_fullscreen_nonBlocking_P(
            _i("Autoloading filament available only when filament sensor is turned on..."),
            nlines); ////MSG_AUTOLOADING_ONLY_IF_FSENS_ON c=20 r=4
    tim = _millis();
  }
  menu_back_if_clicked();
}
...
static void lcd_fsensor_fail() {
  ...
  if ((tim + 1000) < _millis()) {
    lcd_display_message_fullscreen_nonBlocking_P(
            _i("ERROR: Filament sensor is not responding, please check connection."),
...
void lcd_first_layer_calibration_reset() {
  ...
  if (LCD_CLICKED || !eeprom_is_sheet_initialized(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet))) ||
      ...
      (0 == static_cast<int16_t>(eeprom_read_word(reinterpret_cast<uint16_t *>
                                                  (&EEPROM_Sheets_base->s[(eeprom_read_byte(
                      &(EEPROM_Sheets_base->active_sheet)))].z_offset))))) {
    if (menuData->reset) {
      eeprom_update_word(reinterpret_cast<uint16_t *>(&EEPROM_Sheets_base->s[(eeprom_read_byte(
              &(EEPROM_Sheets_base->active_sheet)))].z_offset), 0xffff);
    }
  ...
  eeprom_read_block(sheet_name, &EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].name,
                    sizeof(Sheet::name));
  lcd_set_cursor(0, 0);
  float offset = static_cast<int16_t>(eeprom_read_word(
          reinterpret_cast<uint16_t *>(&EEPROM_Sheets_base->s[(eeprom_read_byte(
                  &(EEPROM_Sheets_base->active_sheet)))].z_offset))) / cs.axis_steps_per_unit[Z_AXIS];
  lcd_printf_P(
...
void lcd_v2_calibration() {
  if (mmu_enabled) {
    ...
    if (fsensor_enabled && ir_sensor_detected) {
      ...
      loaded = lcd_show_fullscreen_message_yes_no_and_wait_P(_i("Is filament loaded?"), false,
                                                             true);////MSG_PLA_FILAMENT_LOADED c=20 r=2
...
void lcd_wizard() {
  ...
  if (calibration_status() != CALIBRATION_STATUS_ASSEMBLED) {
    result = lcd_show_multiscreen_message_yes_no_and_wait_P(
            _i("Running Wizard will delete current calibration results and start from the beginning. Continue?"), false,
...
static void lcd_wizard_load() {
  if (mmu_enabled) {
    ...
    lcd_show_fullscreen_message_and_wait_P(
            _i("Please insert filament into the extruder, then press the knob to load it."));////MSG_WIZARD_LOAD_FILAMENT c=20 r=8
  }
  lcd_update_enable(false);
  lcd_clear();
    ...
    switch (state) {
      case S::Run: //Run wizard?
        ...
        wizard_event = lcd_show_multiscreen_message_yes_no_and_wait_P(
                _i("Hi, I am your Original Prusa i3 printer. Would you like me to guide you through the setup process?"),
                false, true);////MSG_WIZARD_WELCOME c=20 r=7
        if (wizard_event) {
          state = S::Restore;
      ...
      case S::IsFil:
        ...
        if (mmu_enabled) {
          ...
          wizard_event = lcd_show_fullscreen_message_yes_no_and_wait_P(_i("Is filament loaded?"),
                                                                       true);////MSG_WIZARD_FILAMENT_LOADED c=20 r=2
        }
      ...
      case S::RepeatLay1Cal:
        ...
        if (wizard_event) {
          ...
          lcd_show_fullscreen_message_and_wait_P(
                  _i("If you have additional steel sheets, calibrate their presets in Settings - HW Setup - Steel sheets."));
          state = S::Finish;
        }
        break;
      case S::Finish:
...
{\
    ...
    switch(oNozzleDiameter)\
    {\
        case ClNozzleDiameter::_Diameter_100: fNozzleDiam = 0.1f; break;\
        case ClNozzleDiameter::_Diameter_200: fNozzleDiam = 0.2f; break;\
        ...
        case ClNozzleDiameter::_Diameter_700: fNozzleDiam = 0.7f; break;\
        case ClNozzleDiameter::_Diameter_800: fNozzleDiam = 0.8f; break;\
        case ClNozzleDiameter::_Diameter_900: fNozzleDiam = 0.9f; break;\
        case ClNozzleDiameter::_Diameter_1000: fNozzleDiam = 1.0f; break;\
        case ClNozzleDiameter::_Diameter_1100: fNozzleDiam = 1.1f; break;\
        case ClNozzleDiameter::_Diameter_1200: fNozzleDiam = 1.2f; break;\
...
static void lcd_support_menu() {
  ...
  MENU_BEGIN();
    ...
    MENU_ITEM_BACK_P(PSTR(" G2/G3 Fix"));
...
static void lcd_settings_menu() {
  ...
  MENU_BEGIN();
    ...
    if (has_temperature_compensation()) {
      MENU_ITEM_TOGGLE_P(_T(MSG_TEMP_CALIBRATION),
...
static void lcd_disable_farm_mode() {
  int8_t disable = lcd_show_fullscreen_message_yes_no_and_wait_P(PSTR("Disable farm mode?"), true,
                                                                 false); //allow timeouting, default no
  if (disable) {
...
void lcd_resume_print() {
  ...
  pause_time += (_millis() -
                 start_pause_print); //accumulate time when print is paused for correct statistics calculation
  refresh_cmd_timeout();
...
static void lcd_main_menu() {
 ...
 MENU_ITEM_FUNCTION_P(PSTR("power panic"), uvlo_);
    ...
    if ((IS_SD_PRINTING || is_usb_printing || (lcd_commands_type == LcdCommands::Layer1Cal)) &&
        (current_position[Z_AXIS] < Z_HEIGHT_HIDE_LIVE_ADJUST_MENU) && !homing_flag && !mesh_bed_leveling_flag) {
    ...
    if (card.cardOK || lcd_commands_type == LcdCommands::Layer1Cal) {
      ...
      } else if (lcd_commands_type == LcdCommands::Layer1Cal && mesh_bed_leveling_flag == false &&
                 homing_flag == false) {
        //MENU_ITEM_SUBMENU_P(_T(MSG_STOP_PRINT), lcd_sdcard_stop);
      } else {
...
static void lcd_backlight_menu() {
  MENU_BEGIN();
    ...
    MENU_ITEM_TOGGLE_P(_T(MSG_MODE), ((backlightMode == BACKLIGHT_MODE_BRIGHT) ? _T(MSG_BRIGHT) : ((backlightMode ==
                                                                                                    BACKLIGHT_MODE_DIM)
                                                                                                   ? _T(MSG_DIM) : _T(
                    MSG_AUTO))), backlight_mode_toggle);
...
static bool lcd_selftest_IRsensor(bool bStandalone) {
  ...
  lcd_show_fullscreen_message_and_wait_P(
          _i("Insert the filament (do not load it) into the extruder and then press the knob."));////c=20 r=6
  volt_IR_int = current_voltage_raw_IR;
...
bool lcd_selftest() {
  ...
  lcd_clear();
  lcd_set_cursor(0, 0);
  lcd_puts_P(_i("Self test start  "));////MSG_SELFTEST_START c=20
#ifdef TMC2130
  FORCE_HIGH_POWER_START;
#endif // TMC2130
  FORCE_BL_ON_START;
...
//end of second measurement, now check for possible errors:
  ...
  if (abs(measured_axis_length[0] - measured_axis_length[1]) >
...
static bool lcd_selfcheck_check_heater(bool _isbed) {
  ...
  do {
    ...
    manage_heater();
    manage_inactivity(true);
    _progress = (_isbed) ? lcd_selftest_screen(TestScreen::Bed, _progress, 2, false, 400) : lcd_selftest_screen(
            TestScreen::Hotend, _progress, 2, false, 400);
    /*if (_isbed) {
...
static int lcd_selftest_screen(TestScreen screen, int _progress, int _progress_scale, bool _clear, int _delay) {
  ...
  if ((screen >= TestScreen::ExtruderFan) && (screen <= TestScreen::FansOk)) {
    ...
    lcd_selftest_screen_step(3, 9, ((screen == _step_block) ? 1 : (screen < _step_block) ? 0 : 2), "Hotend",
                             _indicator);
  }
...
static void menu_action_sdfile(const char *filename) {
  ...
  if (!check_file(filename)) {
    result = lcd_show_fullscreen_message_yes_no_and_wait_P(_i("File incomplete. Continue anyway?"), false,
...
void lcd_ping() { //chceck if printer is connected to monitoring when in farm mode
  if (farm_mode) {
    ...
    if ((_millis() - PingTime) * 0.001 >
        (empty ? PING_TIME : PING_TIME_LONG)) { //if commands buffer is empty use shorter time period
